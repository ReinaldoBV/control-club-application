import { ComponentFixture, TestBed } from '@angular/core/testing';
import { provideRouter, withComponentInputBinding } from '@angular/router';
import { RouterTestingHarness } from '@angular/router/testing';
import { of } from 'rxjs';

import { PartidoDetailComponent } from './partido-detail.component';

describe('Partido Management Detail Component', () => {
  let comp: PartidoDetailComponent;
  let fixture: ComponentFixture<PartidoDetailComponent>;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [PartidoDetailComponent],
      providers: [
        provideRouter(
          [
            {
              path: '**',
              component: PartidoDetailComponent,
              resolve: { partido: () => of({ id: 123 }) },
            },
          ],
          withComponentInputBinding(),
        ),
      ],
    })
      .overrideTemplate(PartidoDetailComponent, '')
      .compileComponents();
  });

  beforeEach(() => {
    fixture = TestBed.createComponent(PartidoDetailComponent);
    comp = fixture.componentInstance;
  });

  describe('OnInit', () => {
    it('Should load partido on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', PartidoDetailComponent);

      // THEN
      expect(instance.partido()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });

  describe('PreviousState', () => {
    it('Should navigate to previous state', () => {
      jest.spyOn(window.history, 'back');
      comp.previousState();
      expect(window.history.back).toHaveBeenCalled();
    });
  });
});
