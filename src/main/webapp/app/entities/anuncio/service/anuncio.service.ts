...
type RestOf<T extends IAnuncio | NewAnuncio> = Omit<T, 'fechaPublicacion'> & {
...
export class AnuncioService {
  ...
  protected convertDateFromClient<T extends IAnuncio | NewAnuncio | PartialUpdateAnuncio>(anuncio: T): RestOf<T> {
    return {
      ...anuncio,
      fechaPublicacion: anuncio.fechaPublicacion?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAnuncio: RestAnuncio): IAnuncio {
    return {
      ...restAnuncio,
      fechaPublicacion: restAnuncio.fechaPublicacion ? dayjs(restAnuncio.fechaPublicacion) : undefined,
    };
  }
